...
export const useVideoExplorer = () => {
  ...
  const calculateMetrics = (rawVideos: any[]): VideoExplorerData[] => {
    return rawVideos.map(video => {
      ...
      // Basic performance score (can be enhanced)
      const performance_score = (engagement_rate + retention_rate + saves_per_1k + for_you_percentage) / 4;
  ...
  const loadVideos = async () => {
    ...
    try {
      const { data, error } = await (supabase as any)
        .from('videos')
        .select(`
          id, title, image_url, video_url, views, likes, comments, shares, saves,
